...
namespace RedGifsDownloader
{
    public static class RedGifs
    {
        ...
        internal static void DownloadUser(string userId, bool downloadMp4, int minLikes)
        {
            ...
            var user = JObject.Parse(WebClient.DownloadString($"{Api.BaseUrl}{Api.UsersEndpoint}{userId}{Api.UserGfysEndpoint}?count=100"));
            ...
            if (user != null)
            {
                foreach (var gif in user["gfycats"])
                {
                    if ((int)gif["likes"] >= minLikes)
                    {
                        ...
                        if (downloadMp4)
                        {
                            ...
                            {
                                WebClient.DownloadFile((string)gif["mobileUrl"], userId + "/" + (string)gif["gfyName"] + ".mp4");
                            }
                        }
                        else
                        {
                            if (gif["content_urls"] != null && gif["content_urls"]["largeGif"] != null &&
                                ...
                                WebClient.DownloadFile((string)gif["gifUrl"], userId + "/" + (string)gif["gfyName"] + ".gif");
                        }
                    }   
                }
                ...
                {
                    user = JObject.Parse(WebClient.DownloadString($"{Api.BaseUrl}{Api.UsersEndpoint}{userId}{Api.UserGfysEndpoint}?count=100&cursor={(string)user["cursor"]}"));
                    foreach (var gif in user["gfycats"])
                    {
                        if ((int)gif["likes"] >= minLikes)
                        {
        ...
        internal static void DownloadBySearch(string searchTerm, bool downloadMp4, int minLikes)
        {
            ...
            var search = JObject.Parse(WebClient.DownloadString($"{Api.BaseUrl}{Api.SearchEndpoint}?search_text={searchTerm}&count=150&order=trending"));
            if (search != null)
            {
                foreach (var gif in search["gfycats"])
                {
                    if ((int)gif["likes"] >= minLikes)
                    {
                        ...
                        if (downloadMp4)
                        {
                            try
                            {
                                WebClient.DownloadFile((string)gif["mp4Url"] ?? (string)gif["mobileUrl"], "search/" + searchTerm + "/" + (string)gif["gfyName"] + ".mp4");
                            }
                            catch
                            {
                                WebClient.DownloadFile((string)gif["mobileUrl"], "search/" + searchTerm + "/" + (string)gif["gfyName"] + ".mp4");
                            }
                        }
                        else
                        {
                            if (gif["content_urls"] != null && gif["content_urls"]["largeGif"] != null && gif["content_urls"]["largeGif"]["url"] != null)
                                WebClient.DownloadFile((string)gif["content_urls"]["largeGif"]["url"], "search/" + searchTerm + "/" + (string)gif["gfyName"] + ".gif");
                            else if (gif["gifUrl"] != null)
                                WebClient.DownloadFile((string)gif["gifUrl"],  "search/" + searchTerm + "/" + (string)gif["gfyName"] + ".gif");
                        }
                    }   
                }
                ...
                {
                    search = JObject.Parse(WebClient.DownloadString($"{Api.BaseUrl}{Api.SearchEndpoint}?search_text={searchTerm}&count=150&order=trending&cursor={(string)search["cursor"]}"));
                    foreach (var gif in search["gfycats"])
                    {
                        if ((int)gif["likes"] >= minLikes)
                        {
